      ...
      {!plan && (
        <section className="bg-white/40 glass-card rounded-[48px] p-10 border-white shadow-3xl shadow-indigo-100/50 space-y-10">
            <div className="space-y-4">
                <h2 className="text-4xl font-black text-indigo-900 leading-[0.9] tracking-tighter">Plan your <span className="text-transparent bg-clip-text gradient-indigo">family joy</span></h2>
                <p className="text-gray-400 font-bold text-[10px] uppercase tracking-widest">Village Friendly Meal Planning</p>
            </div>
            
            <div className="space-y-6">
                <label className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] flex items-center gap-3">
                    <IndianRupee className="w-5 h-5 p-1 bg-indigo-50 rounded-lg" /> Daily Budget
                </label>
            ...
            <div className="space-y-4">
                <label className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] flex items-center gap-3">
                    <Target className="w-5 h-5 p-1 bg-indigo-50 rounded-lg" /> Health Focus
                </label>
            ...
            >
                {loading ? <RefreshCw className="w-6 h-6 animate-spin" /> : <><Sparkles className="w-6 h-6" /> Create Menu ❤️</>}
            </button>
        </section>
      )}
        ...
        {plan && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
                <div className="flex items-center justify-between px-2">
                    <h2 className="text-3xl font-black text-indigo-950 tracking-tighter">Your Joy Menu</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <MealCard icon={<Coffee className="w-6 h-6 md:w-8 md:h-8" />} label="Breakfast" meal={plan.breakfast} color="orange" />
                    <MealCard icon={<Sun className="w-6 h-6 md:w-8 md:h-8" />} label="Lunch" meal={plan.lunch} color="emerald" />
                    <MealCard icon={<Cookie className="w-6 h-6 md:w-8 md:h-8" />} label="Snack" meal={plan.snacks} color="sky" />
                    <MealCard icon={<Moon className="w-6 h-6 md:w-8 md:h-8" />} label="Dinner" meal={plan.dinner} color="indigo" />
                </div>
